...
export async function loadBlob<T>(key: string): Promise<T | null> {
  try {
    const record = await prisma.pumpshotCache.findUnique({
      where: { id: key },
    });
    if (record) {
    ...
    console.error(`[store] loadBlob error for ${key}:`, err);
  }
  return null;
}
...
export async function saveBlob(key: string, value: unknown): Promise<void> {
  try {
    await prisma.pumpshotCache.upsert({
      where: { id: key },
      update: {
    ...
    console.error(`[store] saveBlob error for ${key}:`, err);
  }
}
...
const INDEX_MAX = Number(process.env.PUMPSHOT_INDEX_MAX ?? 60);
