...
import { upsertTrackedWallet, syncPositionBook } from "./db";
...
async function walletFills(
  ...
  for (const tx of txs) {
    ...
    for (const after of tx.postTokenBalances) {
      ...
      base +=
        Number(after.amountRaw - (before?.amountRaw ?? BigInt(0))) / 10 ** after.decimals;
    }
    /**
    ...
    for (const after of tx.postTokenBalances) {
      ...
      const delta = after.amountRaw - (before?.amountRaw ?? BigInt(0));
      if (delta === BigInt(0)) continue;
      if (base > 0 ? delta < BigInt(0) : delta > BigInt(0)) counterparties.push(after.owner);
    }
    moves.push({ ts, base, counterparties, signature: tx.signature });
  }
...
async function walletHistory(
  ...
  void upsertTrackedWallet(wallet);
  void syncPositionBook(mint, book);
